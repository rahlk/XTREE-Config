...
class patches:
    ...
    def main(i, config=False, justDeltas=False):
        newRows = [i.patchIt(i.testDF.iloc[n], config) for n in xrange(i.testDF.shape[0]) if
        ...
        gain = (1 - sum(after) / sum(i.testDF[i.testDF.columns[-1]])) * 100 if i.config else (1 - sum(after) / len(
            after)) * 100
...
def xtree(train, test, rftrain=None, config=False, tunings=None, justDeltas=False):
    ...
    if config:
        ...
        patch = patches(train=train, test=test, trainDF=train_DF, testDF=test_DF, tree=tree, tunings=tunings,
                        config=True)
        ...
        # set_trace()
        patch = patches(train=train, test=test, trainDF=train_DF, testDF=test_DF, rfTrain=rftrain, tunings=tunings,
                        tree=tree)
